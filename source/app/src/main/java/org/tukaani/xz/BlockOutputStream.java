...
class BlockOutputStream extends FinishableOutputStream {
    ...
    public BlockOutputStream(OutputStream outputStream, FilterEncoder[] filterEncoderArr, Check check, ArrayCache arrayCache) throws IOException {
        ...
        for (int i10 = 0; i10 < filterEncoderArr.length; i10++) {
            EncoderUtil.encodeVLI(byteArrayOutputStream, filterEncoderArr[i10].getFilterID());
            byte[] filterProps = filterEncoderArr[i10].getFilterProps();
    ...
    public void write(byte[] bArr, int i10, int i11) throws IOException {
        this.filterChain.write(bArr, i10, i11);
        this.check.update(bArr, i10, i11);
