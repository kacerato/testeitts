...
public class DiskGenerator implements SupportBallGenerator<Euclidean2D, Vector2D> {
    ...
    public EnclosingBall<Euclidean2D, Vector2D> ballOnSupport(List<Vector2D> list) {
        ...
        BigFraction add2 = multiply2.add(bigFraction4.multiply(bigFraction4));
        ...
        BigFraction[] bigFractionArr3 = {add, add2, multiply3.add(bigFraction6.multiply(bigFraction6))};
        ...
        return new EnclosingBall<>(new Vector2D(divide.doubleValue(), negate.doubleValue()), FastMath.sqrt(subtract.multiply(subtract).add(subtract2.multiply(subtract2)).doubleValue()), vector2D, vector2D2, vector2D3);
    }
}
