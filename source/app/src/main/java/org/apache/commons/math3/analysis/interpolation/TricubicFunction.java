...
class TricubicFunction implements TrivariateFunction {
    ...
    private static final short f100129N = 4;

    private final double[][][] f100130a = (double[][][]) Array.newInstance(Double.TYPE, 4, 4, 4);

    public TricubicFunction(double[] dArr) {
        for (int i10 = 0; i10 < 4; i10++) {
            for (int i11 = 0; i11 < 4; i11++) {
                for (int i12 = 0; i12 < 4; i12++) {
                    this.f100130a[i10][i11][i12] = dArr[(((i12 * 4) + i11) * 4) + i10];
                }
            }
        }
    }
    ...
    public double value(double d10, double d11, double d12) throws OutOfRangeException {
        ...
        for (int i10 = 0; i10 < 4; i10++) {
            for (int i11 = 0; i11 < 4; i11++) {
                for (int i12 = 0; i12 < 4; i12++) {
                    d13 += this.f100130a[i10][i11][i12] * dArr[i10] * dArr2[i11] * dArr3[i12];
                }
            }
        }
        return d13;
