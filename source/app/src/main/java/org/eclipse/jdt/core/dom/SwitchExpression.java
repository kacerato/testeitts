...
public class SwitchExpression extends Expression {
    ...
    static {
        ...
        ASTNode.createPropertyList(SwitchExpression.class, arrayList);
        ASTNode.addProperty(childPropertyDescriptor, arrayList);
        ASTNode.addProperty(childListPropertyDescriptor, arrayList);
        PROPERTY_DESCRIPTORS_12_0 = ASTNode.reapPropertyList(arrayList);
    }

    public SwitchExpression(AST ast) {
        super(ast);
    ...
    public ASTNode clone0(AST ast) {
        SwitchExpression switchExpression = new SwitchExpression(ast);
        switchExpression.setSourceRange(getStartPosition(), getLength());
        switchExpression.setExpression((Expression) getExpression().clone(ast));
        switchExpression.statements().addAll(ASTNode.copySubtrees(ast, statements()));
