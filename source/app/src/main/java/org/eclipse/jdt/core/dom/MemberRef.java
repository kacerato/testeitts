...
public class MemberRef extends ASTNode implements IDocElement {
    ...
    static {
        ChildPropertyDescriptor childPropertyDescriptor = new ChildPropertyDescriptor(MemberRef.class, "qualifier", Name.class, false, false);
        QUALIFIER_PROPERTY = childPropertyDescriptor;
        ChildPropertyDescriptor childPropertyDescriptor2 = new ChildPropertyDescriptor(MemberRef.class, "name", SimpleName.class, true, false);
        NAME_PROPERTY = childPropertyDescriptor2;
        ArrayList arrayList = new ArrayList(3);
        ASTNode.createPropertyList(MemberRef.class, arrayList);
        ASTNode.addProperty(childPropertyDescriptor, arrayList);
        ASTNode.addProperty(childPropertyDescriptor2, arrayList);
        PROPERTY_DESCRIPTORS = ASTNode.reapPropertyList(arrayList);
    }

    public MemberRef(AST ast) {
        super(ast);
    ...
    public ASTNode clone0(AST ast) {
        MemberRef memberRef = new MemberRef(ast);
        memberRef.setSourceRange(getStartPosition(), getLength());
        memberRef.setQualifier((Name) ASTNode.copySubtree(ast, getQualifier()));
        memberRef.setName((SimpleName) ASTNode.copySubtree(ast, getName()));
        return memberRef;
    }
    ...
    @Override
    public int treeSize() {
        return memSize() + (this.optionalQualifier == null ? 0 : getQualifier().treeSize()) + (this.memberName != null ? getName().treeSize() : 0);
    }
}
