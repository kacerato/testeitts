package com.android.tools.r8.internal;

import com.android.tools.r8.graph.C4766w5;
import com.android.tools.r8.graph.C4798y;
import java.nio.ShortBuffer;

public class C5174Fq extends AbstractC5981To {
    public C5174Fq(int i10, C5693Op c5693Op) {
        super(i10, c5693Op);
    }

    @Override
    public final void a(C4798y c4798y, AbstractC5308Hz abstractC5308Hz, com.android.tools.r8.dex.M m10, com.android.tools.r8.graph.H5 h52, RR rr) {
    }

    @Override
    public final boolean l() {
        return true;
    }

    @Override
    public final String t() {
        return "MonitorExit";
    }

    @Override
    public final int v() {
        return 30;
    }

    @Override
    public final String z() {
        return "monitor-exit";
    }

    public C5174Fq(int i10) {
        super(i10);
    }

    @Override
    public final void a(C6382aB c6382aB) {
        c6382aB.a(new WX(XX.f45733c, c6382aB.b(this.f44594f, EnumC5592Mw0.f42461b)));
    }

    @Override
    public final void a(C4766w5 c4766w5, com.android.tools.r8.graph.H5 h52, AbstractC5308Hz abstractC5308Hz, AbstractC5308Hz abstractC5308Hz2, RR rr, ShortBuffer shortBuffer) {
        AbstractC5635Np.a(this.f44594f, 30, shortBuffer);
    }
}
