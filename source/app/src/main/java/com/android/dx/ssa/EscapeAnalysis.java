...
public class EscapeAnalysis {
    ...
    private int findSetIndex(RegisterSpec registerSpec) {
        int i10 = 0;
        while (i10 < this.latticeValues.size() && !this.latticeValues.get(i10).regSet.get(registerSpec.getReg())) {
            i10++;
        }
        return i10;
    }
    ...
    private void insertExceptionThrow(SsaInsn ssaInsn, RegisterSpec registerSpec, HashSet<SsaInsn> hashSet) {
        ...
        insertThrowingInsnBefore(ssaInsn3, RegisterSpecList.make(make, registerSpec), null, 52, new CstMethodRef(cstType, new CstNat(new CstString(ConstantDescs.INIT_NAME), new CstString("(I)V"))));
    ...
    private void insertPlainInsnBefore(SsaInsn ssaInsn, RegisterSpecList registerSpecList, RegisterSpec registerSpec, int i10, Constant constant) {
    ...
    private void insertThrowingInsnBefore(SsaInsn ssaInsn, RegisterSpecList registerSpecList, RegisterSpec registerSpec, int i10, Constant constant) {
        Insn originalRopInsn = ssaInsn.getOriginalRopInsn();
        Rop ropFor = Rops.ropFor(i10, registerSpec, registerSpecList, constant);
    ...
    private void movePropagate() {
        for (int i10 = 0; i10 < this.ssaMeth.getRegCount(); i10++) {
    ...
    private void replaceDef(SsaInsn ssaInsn, SsaInsn ssaInsn2, int i10, ArrayList<RegisterSpec> arrayList) {
    ...
    private void replaceUse(SsaInsn ssaInsn, SsaInsn ssaInsn2, ArrayList<RegisterSpec> arrayList, HashSet<SsaInsn> hashSet) {
        ...
        if (opcode == 57) {
            ...
            for (int i10 = 0; i10 < size; i10++) {
                RegisterSpec make = RegisterSpec.make(arrayList.get(i10).getReg(), (TypeBearer) initValues.get(i10));
                insertPlainInsnBefore(ssaInsn, RegisterSpecList.EMPTY, make, 5, initValues.get(i10));
                arrayList.set(i10, make);
            }
            return;
        }
        if (opcode == 38) {
