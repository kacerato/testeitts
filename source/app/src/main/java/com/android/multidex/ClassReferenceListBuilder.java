...
public class ClassReferenceListBuilder {
    ...
    private void addClassWithHierachy(String str) {
        ...
        try {
            ...
            for (int i10 = 0; i10 < size; i10++) {
                addClassWithHierachy(interfaces.getType(i10).getClassName());
            }
    ...
    private void addDependencies(DirectClassFile directClassFile) {
        ...
        for (int i10 = 0; i10 < size; i10++) {
            checkDescriptor(fields.get(i10).getDescriptor().getString());
        }
        MethodList methods = directClassFile.getMethods();
        int size2 = methods.size();
        for (int i11 = 0; i11 < size2; i11++) {
            checkPrototype(Prototype.intern(methods.get(i11).getDescriptor().getString()));
        }
    }
    ...
    private void checkPrototype(Prototype prototype) {
        ...
        for (int i10 = 0; i10 < parameterTypes.size(); i10++) {
            checkDescriptor(parameterTypes.get(i10).getDescriptor());
        }
    }
        ...
        while (entries2.hasMoreElements()) {
            ...
            if (name2.endsWith(".class")) {
                try {
                    ...
                    throw new IOException("Class " + name2 + " is missing form original class path " + ((Object) this.path), e10);
                }
            }
        }
    }
