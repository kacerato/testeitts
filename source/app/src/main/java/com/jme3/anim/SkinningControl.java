...
public class SkinningControl extends AbstractControl implements JmeCloneable {
    ...
    private void applySkinning(Mesh mesh, Matrix4f[] matrix4fArr) {
        ...
        FloatBuffer floatBuffer3 = buffer2 == null ? null : (FloatBuffer) buffer2.getData();
        ...
        while (ceil >= 0) {
            ...
            if (floatBuffer6 != null) {
                ...
                floatBuffer3.position(floatBuffer6.position() - min);
            ...
            buffer2 = vertexBuffer6;
            buffer = vertexBuffer5;
            tempVars2 = tempVars4;
        }
        VertexBuffer vertexBuffer7 = buffer2;
        tempVars2.release();
        buffer.updateData(floatBuffer2);
        if (vertexBuffer7 != null) {
            vertexBuffer7.updateData(floatBuffer3);
        }
    }
    ...
    private void applySoftwareSkinning(Mesh mesh, Matrix4f[] matrix4fArr) {
    ...
    private void collectAnimatedGeometries(Spatial spatial) {
        if (!(spatial instanceof Geometry)) {
            if (spatial instanceof Node) {
                Iterator<Spatial> it = ((Node) spatial).getChildren().iterator();
                while (it.hasNext()) {
                    collectAnimatedGeometries(it.next());
                }
                return;
            }
            return;
    ...
    private void controlRenderHardware() {
        Matrix4f[] computeSkinningMatrices = this.armature.computeSkinningMatrices();
        this.boneOffsetMatrices = computeSkinningMatrices;
        this.jointMatricesParam.setValue(computeSkinningMatrices);
    }

    private void controlRenderSoftware() {
        resetToBind();
        this.boneOffsetMatrices = this.armature.computeSkinningMatrices();
        Iterator<Geometry> it = this.targets.iterator();
        while (it.hasNext()) {
            applySoftwareSkinning(it.next().getMesh(), this.boneOffsetMatrices);
        }
    }

    private void enableHardwareSkinning() {
        this.numberOfJointsParam.setEnabled(true);
        this.jointMatricesParam.setEnabled(true);
        this.numberOfJointsParam.setValue(Integer.valueOf(((this.armature.getJointCount() / 10) + 1) * 10));
        Iterator<Geometry> it = this.targets.iterator();
        while (it.hasNext()) {
    ...
    private void enableSoftwareSkinning() {
        this.numberOfJointsParam.setEnabled(false);
        this.jointMatricesParam.setEnabled(false);
        Iterator<Geometry> it = this.targets.iterator();
        while (it.hasNext()) {
            Mesh mesh = it.next().getMesh();
            if (mesh != null && mesh.isAnimated()) {
    ...
    private boolean testHardwareSupported(RenderManager renderManager) {
        if (this.armature.getJointCount() > 255) {
            logger.log(Level.INFO, "Hardware skinning not supported for {0}: Too many bones ({1} > 255).", new Object[]{this.spatial, Integer.valueOf(this.armature.getJointCount())});
            return false;
        }
        enableHardwareSkinning();
        try {
            renderManager.preloadScene(this.spatial);
            logger.log(Level.INFO, "Hardware skinning engaged for {0}", this.spatial);
            return true;
        } catch (RendererException e10) {
            logger.log(Level.WARNING, "Could not enable HW skinning due to shader compile error: ", (Throwable) e10);
            return false;
        }
    }
    ...
    public void cloneFields(Cloner cloner, Object obj) {
        ...
        this.numberOfJointsParam = (MatParamOverride) cloner.clone(this.numberOfJointsParam);
        this.jointMatricesParam = (MatParamOverride) cloner.clone(this.jointMatricesParam);
    }

    @Override
    public void controlRender(RenderManager renderManager, ViewPort viewPort) {
        if (this.meshUpdateRequired) {
            updateAnimationTargets(this.spatial);
            boolean z10 = this.hwSkinningPreferred;
    ...
    public Node getAttachmentsNode(String str) {
        ...
        if (joint != null) {
            updateAnimationTargets(this.spatial);
            Node attachmentsNode = joint.getAttachmentsNode(this.armature.getJointIndex(joint), this.targets);
        ...
        throw new IllegalArgumentException("Given joint name '" + str + "' does not exist in the armature.");
    }
    ...
    public boolean isHardwareSkinningPreferred() {
        return this.hwSkinningPreferred;
    }
    ...
    public void read(JmeImporter jmeImporter) throws IOException {
        ...
        this.armature = (Armature) jmeImporter.getCapsule(this).readSavable(h.f94106x, null);
        for (MatParamOverride matParamOverride : this.spatial.getLocalMatParamOverrides().getArray()) {
            if (matParamOverride.getName().equals("NumberOfBones") || matParamOverride.getName().equals("BoneMatrices")) {
                this.spatial.removeMatParamOverride(matParamOverride);
            }
        }
        this.spatial.addMatParamOverride(this.numberOfJointsParam);
        this.spatial.addMatParamOverride(this.jointMatricesParam);
    }
    ...
    public void setSpatial(Spatial spatial) {
        ...
        if (spatial2 != null) {
            spatial2.removeMatParamOverride(this.numberOfJointsParam);
            spatial2.removeMatParamOverride(this.jointMatricesParam);
        }
        if (spatial != null) {
            spatial.addMatParamOverride(this.numberOfJointsParam);
            spatial.addMatParamOverride(this.jointMatricesParam);
        }
    }
    ...
    public void write(JmeExporter jmeExporter) throws IOException {
        ...
        jmeExporter.getCapsule(this).write(this.armature, h.f94106x, (Savable) null);
    }
