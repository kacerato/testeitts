...
public class ConstraintSet {
                            ...
                            default:
                                switch (i11) {
                                    ...
                                    case 90:
                                        ...
                                        break;
                                    case 91:
                                        Log.w(ConstraintSet.TAG, "unused attribute 0x" + Integer.toHexString(index) + b.f23785c + mapToConstant.get(index));
                                        break;
                                    default:
                                        Log.w(ConstraintSet.TAG, "Unknown attribute 0x" + Integer.toHexString(index) + b.f23785c + mapToConstant.get(index));
                                        break;
                                }
                        }
                }
    ...
    public class WriteXmlEngine {
        ...
        private void writeEnum(String dimString, int val, String[] types, int def) throws IOException {
            if (val != def) {
                this.writer.write(SPACE + dimString + "=\"" + types[val] + JavadocConstants.ANCHOR_PREFIX_END);
            }
        }
        ...
        public void writeLayout() throws IOException {
            ...
            for (Integer num : ConstraintSet.this.mConstraints.o()) {
                ...
                writeEnum("app:barrierDirection", layout.mBarrierDirection, new String[]{m.f88234c, "right", "top", "bottom", "start", "end"}, -1);
        ...
        public void writeXmlConstraint(String str, int leftToLeft) throws IOException {
            ...
            this.writer.write("=\"" + getName(leftToLeft) + JavadocConstants.ANCHOR_PREFIX_END);
        }
        ...
        public void writeVariable(String name, float value, float def) throws IOException {
            ...
            this.writer.write("=\"" + value + JavadocConstants.ANCHOR_PREFIX_END);
        }
    ...
    private int[] convertReferenceString(View view, String referenceIdString) {
        ...
        String[] split = referenceIdString.split(DocLint.SEPARATOR);
    ...
    public static String getDebugName(int v10) {
        for (Field field : ConstraintSet.class.getDeclaredFields()) {
            if (field.getName().contains(ConstantDescs.DEFAULT_NAME) && field.getType() == Integer.TYPE && Modifier.isStatic(field.getModifiers()) && Modifier.isFinal(field.getModifiers())) {
                try {
    ...
    public static String getLine(Context context, int resourceId, XmlPullParser pullParser) {
        return ".(" + Debug.getName(context, resourceId) + ".xml:" + pullParser.getLineNumber() + ") \"" + pullParser.getName() + JavadocConstants.ANCHOR_PREFIX_END;
    }
    ...
    */
    ...
    private void populateConstraint(Context ctx, Constraint c10, TypedArray a10, boolean override) {
        ...
        for (int i10 = 0; i10 < indexCount; i10++) {
            ...
            switch (mapToConstant.get(index)) {
                ...
                case 87:
                    Log.w(TAG, "unused attribute 0x" + Integer.toHexString(index) + b.f23785c + mapToConstant.get(index));
                    break;
                case 88:
                case 89:
                case 90:
                default:
                    Log.w(TAG, "Unknown attribute 0x" + Integer.toHexString(index) + b.f23785c + mapToConstant.get(index));
                    break;
                case 91:
        ...
        for (Constraint constraint : cs.mConstraints.values()) {
            if (constraint.mDelta != null) {
                if (constraint.mTargetString != null) {
                    Iterator<Integer> it = this.mConstraints.o().iterator();
                    while (it.hasNext()) {
    ...
    public void applyToInternal(ConstraintLayout constraintLayout, boolean applyPostLayout) {
        int childCount = constraintLayout.getChildCount();
        HashSet hashSet = new HashSet(this.mConstraints.o());
        for (int i10 = 0; i10 < childCount; i10++) {
            View childAt = constraintLayout.getChildAt(i10);
            int id2 = childAt.getId();
            if (!this.mConstraints.containsKey(Integer.valueOf(id2))) {
    ...
    public void dump(MotionScene scene, int... ids) {
        HashSet hashSet;
        Set<Integer> o10 = this.mConstraints.o();
        if (ids.length != 0) {
            hashSet = new HashSet();
            for (int i10 : ids) {
    ...
    public int[] getKnownIds() {
        Integer[] numArr = (Integer[]) this.mConstraints.o().toArray(new Integer[0]);
    ...
    public void parseColorAttributes(Constraint set, String attributes) {
        String[] split = attributes.split(DocLint.SEPARATOR);
    ...
    public void readFallback(ConstraintSet set) {
        for (Integer num : set.mConstraints.o()) {
            num.intValue();
            ...
            if (constraint2 != null) {
                ...
                for (String str : constraint.mCustomConstraints.o()) {
    ...
    public void clone(ConstraintSet set) {
        ...
        for (Integer num : set.mConstraints.o()) {
            Constraint constraint = set.mConstraints.get(num);
            if (constraint != null) {
                this.mConstraints.put(num, constraint.m677clone());
            }
        }
    }
