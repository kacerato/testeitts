...
public class MotionConstrainedPoint implements Comparable<MotionConstrainedPoint> {
            ...
            switch (c10) {
                ...
                default:
                    if (str.startsWith("CUSTOM")) {
                        String str2 = str.split(DocLint.SEPARATOR)[1];
                        if (this.attributes.containsKey(str2)) {
                            ...
                            if (viewSpline instanceof ViewSpline.CustomSet) {
                                ...
                                Log.e("MotionPaths", str + " ViewSpline not a CustomSet frame = " + mFramePosition + ", value" + constraintAttribute.getValueToInterpolate() + ((Object) viewSpline));
                                break;
                            }
                        } else {
                            break;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        int i10 = 0;
        float[] fArr = {this.position, this.f32557x, this.f32558y, this.width, this.height, this.alpha, this.elevation, this.rotation, this.rotationX, this.rotationY, this.scaleX, this.scaleY, this.mPivotX, this.mPivotY, this.translationX, this.translationY, this.translationZ, this.mPathRotate};
        for (int i11 : toUse) {
            if (i11 < 18) {
                data[i10] = fArr[r5];
                i10++;
    ...
    public void different(MotionConstrainedPoint points, boolean[] mask, String[] custom) {
        ...
        mask[1] = mask[1] | diff(this.f32557x, points.f32557x);
        mask[2] = mask[2] | diff(this.f32558y, points.f32558y);
