...
public class ViewTransition {
    ...
    public void lambda$applyTransition$0(View[] viewArr) {
        if (this.mSetsTag != -1) {
    ...
    private void updateTransition(MotionScene.Transition transition, View view) {
        ...
        if (keyFrames != null) {
            ...
            while (it.hasNext()) {
                keyFrames2.addKey(it.next().mo676clone().setViewId(id2));
            }
    ...
    public void applyTransition(ViewTransitionController controller, MotionLayout layout, int fromId, ConstraintSet current, final View... views) {
        ...
        layout.updateState(i12, current);
        ...
        layout.transitionToEnd(new Runnable() {
            @Override
