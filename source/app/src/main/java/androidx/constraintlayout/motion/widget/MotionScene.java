...
public class MotionScene {
    ...
    private int getId(Context context, String idString) {
        ...
        if (idString.contains("/")) {
            i10 = context.getResources().getIdentifier(idString.substring(idString.indexOf(47) + 1), "id", context.getPackageName());
    ...
    private int parseConstraintSet(Context context, XmlPullParser parser) {
        ...
        for (int i12 = 0; i12 < attributeCount; i12++) {
            ...
            String attributeValue = parser.getAttributeValue(i12);
            if (this.DEBUG_DESKTOP) {
                    ...
                    try {
                        ...
                        switch (attributeValue.hashCode()) {
                            case -768416914:
                                ...
                                break;
                            case 3317767:
                                if (attributeValue.equals(m.f88234c)) {
                                    c11 = 1;
                                    break;
                                }
                                break;
    ...
    */
    public void setTransition(int beginId, int endId) {
        ...
        StateSet stateSet = this.mStateSet;
        if (stateSet != null) {
            i10 = stateSet.stateGetConstraintID(beginId, -1, -1);
            if (i10 == -1) {
                i10 = beginId;
            }
            i11 = this.mStateSet.stateGetConstraintID(endId, -1, -1);
        } else {
            i10 = beginId;
        }
        i11 = endId;
        ...
        while (it.hasNext()) {
            Transition next = it.next();
            if ((next.mConstraintSetEnd == i11 && next.mConstraintSetStart == i10) || (next.mConstraintSetEnd == endId && next.mConstraintSetStart == beginId)) {
