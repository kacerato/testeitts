...
public class LifecycleRegistry extends Lifecycle {
    ...
    private void backwardPass(LifecycleOwner lifecycleOwner) {
        ...
        while (descendingIterator.hasNext() && !this.mNewEventOccurred) {
            ...
            while (value.mState.compareTo(this.mState) > 0 && !this.mNewEventOccurred && this.mObserverMap.contains(next.getKey())) {
                ...
                if (downFrom == null) {
                    throw new IllegalStateException("no event down from " + ((Object) value.mState));
                }
    ...
    private void forwardPass(LifecycleOwner lifecycleOwner) {
        ...
        while (iteratorWithAdditions.hasNext() && !this.mNewEventOccurred) {
            ...
            while (observerWithState.mState.compareTo(this.mState) < 0 && !this.mNewEventOccurred && this.mObserverMap.contains((LifecycleObserver) next.getKey())) {
                ...
                if (upFrom == null) {
                    throw new IllegalStateException("no event up from " + ((Object) observerWithState.mState));
                }
    ...
    private void moveToState(Lifecycle.State state) {
        ...
        if (state2 == Lifecycle.State.INITIALIZED && state == Lifecycle.State.DESTROYED) {
            throw new IllegalStateException("no event down from " + ((Object) this.mState));
        }
        this.mState = state;
