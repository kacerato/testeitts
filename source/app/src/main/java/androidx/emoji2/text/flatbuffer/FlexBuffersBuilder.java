...
public class FlexBuffersBuilder {
    ...
    private final ReadWriteBuf f32668bb;
    ...
    private int align(int i10) {
        int i11 = 1 << i10;
        int paddingBytes = Value.paddingBytes(this.f32668bb.writePosition(), i11);
        while (true) {
            int i12 = paddingBytes - 1;
            if (paddingBytes == 0) {
                return i11;
            }
            this.f32668bb.put((byte) 0);
            paddingBytes = i12;
        }
    }
    ...
    private Value createKeyVector(int i10, int i11) {
        ...
        while (i12 < this.stack.size()) {
            i12++;
            max = Math.max(max, Value.elemWidth(4, 0, this.stack.get(i12).key, this.f32668bb.writePosition(), i12));
        }
        int align = align(max);
        writeInt(j10, align);
        int writePosition = this.f32668bb.writePosition();
    ...
    private Value createVector(int i10, int i11, int i12, boolean z10, boolean z11, Value value) {
        ...
        int max = Math.max(0, widthUInBits(j10));
        if (value != null) {
            max = Math.max(max, value.elemWidth(this.f32668bb.writePosition(), 0));
            i13 = 3;
        } else {
            i13 = 1;
        }
        ...
        for (int i18 = i11; i18 < this.stack.size(); i18++) {
            i17 = Math.max(i17, this.stack.get(i18).elemWidth(this.f32668bb.writePosition(), i18 + i13));
            if (z10 && i18 == i11) {
        ...
        if (!z10) {
            while (i19 < this.stack.size()) {
                this.f32668bb.put(this.stack.get(i19).storedPackedType(i17));
                i19++;
            }
        }
        if (value != null) {
    ...
    private int putKey(String str) {
        ...
        if ((this.flags & 1) == 0) {
            ...
            this.f32668bb.put(bytes, 0, bytes.length);
            this.f32668bb.put((byte) 0);
        ...
        this.f32668bb.put(bytes2, 0, bytes2.length);
        this.f32668bb.put((byte) 0);
        this.keyPool.put(str, Integer.valueOf(writePosition));
        return writePosition;
    }
    ...
    private Value writeBlob(int i10, byte[] bArr, int i11, boolean z10) {
        ...
        int writePosition = this.f32668bb.writePosition();
        this.f32668bb.put(bArr, 0, bArr.length);
        if (z10) {
            this.f32668bb.put((byte) 0);
        }
    ...
    private void writeDouble(double d10, int i10) {
        if (i10 == 4) {
            this.f32668bb.putFloat((float) d10);
        } else if (i10 == 8) {
            this.f32668bb.putDouble(d10);
        }
    }

    private void writeInt(long j10, int i10) {
        if (i10 == 1) {
            this.f32668bb.put((byte) j10);
            return;
        }
        if (i10 == 2) {
            this.f32668bb.putShort((short) j10);
        } else if (i10 == 4) {
            this.f32668bb.putInt((int) j10);
        } else {
            if (i10 != 8) {
                return;
            }
            this.f32668bb.putLong(j10);
        }
    }

    private void writeOffset(long j10, int i10) {
        writeInt((int) (this.f32668bb.writePosition() - j10), i10);
    }
    ...
    public ByteBuffer finish() {
        int align = align(this.stack.get(0).elemWidth(this.f32668bb.writePosition(), 0));
        writeAny(this.stack.get(0), align);
        this.f32668bb.put(this.stack.get(0).storedPackedType());
        this.f32668bb.put((byte) align);
        this.finished = true;
        return ByteBuffer.wrap(this.f32668bb.data(), 0, this.f32668bb.writePosition());
    }

    public ReadWriteBuf getBuffer() {
        return this.f32668bb;
    }
    ...
    public FlexBuffersBuilder(ReadWriteBuf readWriteBuf, int i10) {
        ...
        this.keyComparator = new Comparator<Value>() {
            ...
            public int compare(Value value, Value value2) {
                ...
                int i12 = value2.key;
                do {
                    b10 = FlexBuffersBuilder.this.f32668bb.get(i11);
                    b11 = FlexBuffersBuilder.this.f32668bb.get(i12);
                    if (b10 == 0) {
                        return b10 - b11;
                    }
                    i11++;
