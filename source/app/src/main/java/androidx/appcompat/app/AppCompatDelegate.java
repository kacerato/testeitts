...
public abstract class AppCompatDelegate {
    ...
    public static Object getLocaleManagerForApplication() {
        ...
        while (it.hasNext()) {
            ...
            if (appCompatDelegate != null && (contextForDelegate = appCompatDelegate.getContextForDelegate()) != null) {
                return contextForDelegate.getSystemService(UserDictionary.Words.LOCALE);
            }
        }
        return null;
    }
    ...
    public static void lambda$syncRequestedAndStoredLocales$1(Context context) {
    ...
    public static void syncRequestedAndStoredLocales(final Context context) {
        if (isAutoStorageOptedIn(context)) {
            if (BuildCompat.isAtLeastT()) {
                ...
                sSerialExecutorForLocalesStorage.execute(new Runnable() {
                    @Override
                    public final void run() {
                        AppCompatDelegate.lambda$syncRequestedAndStoredLocales$1(Context.this);
                    }
                });
                return;
            }
    ...
    public void asyncExecuteSyncRequestedAndStoredLocales(final Context context) {
        sSerialExecutorForLocalesStorage.execute(new Runnable() {
            @Override
            public final void run() {
                AppCompatDelegate.syncRequestedAndStoredLocales(Context.this);
            }
        });
    }
