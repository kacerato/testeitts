.class public final LBi/r;
.super Ljava/lang/Object;
.source "SourceFile"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        LBi/r$d;,
        LBi/r$e;
    }
.end annotation


# static fields
.field public static final a:Ljava/util/logging/Logger;

.field public static final b:Ljava/security/Permission;

.field public static final c:Ljava/security/Permission;

.field public static final d:Ljava/security/Permission;

.field public static final e:Ljava/security/Permission;

.field public static final f:Ljava/lang/ThreadLocal;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/lang/ThreadLocal<",
            "Ljava/util/Map<",
            "Ljava/lang/String;",
            "[",
            "Ljava/lang/Object;",
            ">;>;"
        }
    .end annotation
.end field

.field public static final g:Ljava/util/Map;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/Map<",
            "Ljava/lang/String;",
            "[",
            "Ljava/lang/Object;",
            ">;"
        }
    .end annotation
.end field

.field public static final h:LBi/Q;

.field public static final i:LBi/p;

.field public static final j:Ljava/util/concurrent/atomic/AtomicReference;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/concurrent/atomic/AtomicReference<",
            "LBi/Q;",
            ">;"
        }
    .end annotation
.end field

.field public static final k:Z

.field public static final l:Ljava/util/concurrent/atomic/AtomicReference;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/concurrent/atomic/AtomicReference<",
            "LBi/p;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method static constructor <clinit>()V
    .locals 12

    const-class v0, LBi/r;

    invoke-virtual {v0}, Ljava/lang/Class;->getName()Ljava/lang/String;

    move-result-object v0

    invoke-static {v0}, Ljava/util/logging/Logger;->getLogger(Ljava/lang/String;)Ljava/util/logging/Logger;

    move-result-object v0

    sput-object v0, LBi/r;->a:Ljava/util/logging/Logger;

    new-instance v0, LBi/q;

    const-string v1, "globalConfig"

    invoke-direct {v0, v1}, LBi/q;-><init>(Ljava/lang/String;)V

    sput-object v0, LBi/r;->b:Ljava/security/Permission;

    new-instance v0, LBi/q;

    const-string v1, "threadLocalConfig"

    invoke-direct {v0, v1}, LBi/q;-><init>(Ljava/lang/String;)V

    sput-object v0, LBi/r;->c:Ljava/security/Permission;

    new-instance v0, LBi/q;

    const-string v1, "defaultRandomConfig"

    invoke-direct {v0, v1}, LBi/q;-><init>(Ljava/lang/String;)V

    sput-object v0, LBi/r;->d:Ljava/security/Permission;

    new-instance v0, LBi/q;

    const-string v1, "constraints"

    invoke-direct {v0, v1}, LBi/q;-><init>(Ljava/lang/String;)V

    sput-object v0, LBi/r;->e:Ljava/security/Permission;

    new-instance v0, Ljava/lang/ThreadLocal;

    invoke-direct {v0}, Ljava/lang/ThreadLocal;-><init>()V

    sput-object v0, LBi/r;->f:Ljava/lang/ThreadLocal;

    new-instance v0, Ljava/util/HashMap;

    invoke-direct {v0}, Ljava/util/HashMap;-><init>()V

    invoke-static {v0}, Ljava/util/Collections;->synchronizedMap(Ljava/util/Map;)Ljava/util/Map;

    move-result-object v0

    sput-object v0, LBi/r;->g:Ljava/util/Map;

    new-instance v0, LBi/r$e;

    const/4 v1, 0x0

    invoke-direct {v0, v1}, LBi/r$e;-><init>(LBi/r$a;)V

    sput-object v0, LBi/r;->h:LBi/Q;

    new-instance v0, LBi/r$a;

    invoke-direct {v0}, LBi/r$a;-><init>()V

    sput-object v0, LBi/r;->i:LBi/p;

    new-instance v1, Ljava/util/concurrent/atomic/AtomicReference;

    invoke-direct {v1}, Ljava/util/concurrent/atomic/AtomicReference;-><init>()V

    sput-object v1, LBi/r;->j:Ljava/util/concurrent/atomic/AtomicReference;

    new-instance v1, Ljava/util/concurrent/atomic/AtomicReference;

    invoke-direct {v1}, Ljava/util/concurrent/atomic/AtomicReference;-><init>()V

    sput-object v1, LBi/r;->l:Ljava/util/concurrent/atomic/AtomicReference;

    new-instance v2, LXi/z;

    new-instance v3, Ljava/math/BigInteger;

    const-string v4, "fca682ce8e12caba26efccf7110e526db078b05edecbcd1eb4a208f3ae1617ae01f35b91a47e6df63413c5e12ed0899bcd132acd50d99151bdc43ee737592e17"

    const/16 v5, 0x10

    invoke-direct {v3, v4, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v4, Ljava/math/BigInteger;

    const-string v6, "962eddcc369cba8ebb260ee6b6a126d9346e38c5"

    invoke-direct {v4, v6, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v6, Ljava/math/BigInteger;

    const-string v7, "678471b27a9cf44ee91a49c5147db1a9aaf244f05a434d6486931d2d14271b9e35030b71fd73da179069b32e2935630e1c2062354d0da20a6c416e50be794ca4"

    invoke-direct {v6, v7, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v7, LXi/C;

    const-string v8, "b869c82b35d70e1b1ff91b28e37a62ecdc34409b"

    invoke-static {v8}, Lem/h;->d(Ljava/lang/String;)[B

    move-result-object v8

    const/16 v9, 0x7b

    invoke-direct {v7, v8, v9}, LXi/C;-><init>([BI)V

    invoke-direct {v2, v3, v4, v6, v7}, LXi/z;-><init>(Ljava/math/BigInteger;Ljava/math/BigInteger;Ljava/math/BigInteger;LXi/C;)V

    new-instance v3, LXi/z;

    new-instance v4, Ljava/math/BigInteger;

    const-string v6, "e9e642599d355f37c97ffd3567120b8e25c9cd43e927b3a9670fbec5d890141922d2c3b3ad2480093799869d1e846aab49fab0ad26d2ce6a22219d470bce7d777d4a21fbe9c270b57f607002f3cef8393694cf45ee3688c11a8c56ab127a3daf"

    invoke-direct {v4, v6, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v6, Ljava/math/BigInteger;

    const-string v7, "9cdbd84c9f1ac2f38d0f80f42ab952e7338bf511"

    invoke-direct {v6, v7, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v7, Ljava/math/BigInteger;

    const-string v8, "30470ad5a005fb14ce2d9dcd87e38bc7d1b1c5facbaecbe95f190aa7a31d23c4dbbcbe06174544401a5b2c020965d8c2bd2171d3668445771f74ba084d2029d83c1c158547f3a9f1a2715be23d51ae4d3e5a1f6a7064f316933a346d3f529252"

    invoke-direct {v7, v8, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v8, LXi/C;

    const-string v9, "77d0f8c4dad15eb8c4f2f8d6726cefd96d5bb399"

    invoke-static {v9}, Lem/h;->d(Ljava/lang/String;)[B

    move-result-object v9

    const/16 v10, 0x107

    invoke-direct {v8, v9, v10}, LXi/C;-><init>([BI)V

    invoke-direct {v3, v4, v6, v7, v8}, LXi/z;-><init>(Ljava/math/BigInteger;Ljava/math/BigInteger;Ljava/math/BigInteger;LXi/C;)V

    new-instance v4, LXi/z;

    new-instance v6, Ljava/math/BigInteger;

    const-string v7, ""

    invoke-direct {v6, v7, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v7, Ljava/math/BigInteger;

    const-string v8, "9760508f15230bccb292b982a2eb840bf0581cf5"

    invoke-direct {v7, v8, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v8, Ljava/math/BigInteger;

    const-string v9, ""

    invoke-direct {v8, v9, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v9, LXi/C;

    const-string v10, "8d5155894229d5e689ee01e6018a237e2cae64cd"

    invoke-static {v10}, Lem/h;->d(Ljava/lang/String;)[B

    move-result-object v10

    const/16 v11, 0x5c

    invoke-direct {v9, v10, v11}, LXi/C;-><init>([BI)V

    invoke-direct {v4, v6, v7, v8, v9}, LXi/z;-><init>(Ljava/math/BigInteger;Ljava/math/BigInteger;Ljava/math/BigInteger;LXi/C;)V

    new-instance v6, LXi/z;

    new-instance v7, Ljava/math/BigInteger;

    const-string v8, ""

    invoke-direct {v7, v8, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v8, Ljava/math/BigInteger;

    const-string v9, "f8183668ba5fc5bb06b5981e6d8b795d30b8978d43ca0ec572e37e09939a9773"

    invoke-direct {v8, v9, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v9, Ljava/math/BigInteger;

    const-string v10, ""

    invoke-direct {v9, v10, v5}, Ljava/math/BigInteger;-><init>(Ljava/lang/String;I)V

    new-instance v5, LXi/C;

    const-string v10, "b0b4417601b59cbc9d8ac8f935cadaec4f5fbb2f23785609ae466748d9b5a536"

    invoke-static {v10}, Lem/h;->d(Ljava/lang/String;)[B

    move-result-object v10

    const/16 v11, 0x1f1

    invoke-direct {v5, v10, v11}, LXi/C;-><init>([BI)V

    invoke-direct {v6, v7, v8, v9, v5}, LXi/z;-><init>(Ljava/math/BigInteger;Ljava/math/BigInteger;Ljava/math/BigInteger;LXi/C;)V

    sget-object v5, LBi/r$d;->e:LBi/r$d;

    filled-new-array {v2, v3, v4, v6}, [LXi/z;

    move-result-object v7

    invoke-static {v5, v7}, LBi/r;->n(LBi/r$d;[Ljava/lang/Object;)V

    sget-object v5, LBi/r$d;->d:LBi/r$d;

    invoke-static {v2}, LBi/r;->v(LXi/z;)LXi/q;

    move-result-object v2

    invoke-static {v3}, LBi/r;->v(LXi/z;)LXi/q;

    move-result-object v3

    invoke-static {v4}, LBi/r;->v(LXi/z;)LXi/q;

    move-result-object v4

    invoke-static {v6}, LBi/r;->v(LXi/z;)LXi/q;

    move-result-object v6

    filled-new-array {v2, v3, v4, v6}, [LXi/q;

    move-result-object v2

    invoke-static {v5, v2}, LBi/r;->n(LBi/r$d;[Ljava/lang/Object;)V

    invoke-static {}, LBi/r;->f()LBi/p;

    move-result-object v2

    invoke-virtual {v1, v2}, Ljava/util/concurrent/atomic/AtomicReference;->set(Ljava/lang/Object;)V

    invoke-virtual {v1}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v1

    if-eq v1, v0, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    sput-boolean v0, LBi/r;->k:Z

    return-void
.end method

.method public constructor <init>()V
    .locals 0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method public static a(LBi/n;)V
    .locals 1

    sget-object v0, LBi/r;->l:Ljava/util/concurrent/atomic/AtomicReference;

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, LBi/p;

    invoke-interface {v0, p0}, LBi/p;->a(LBi/n;)V

    return-void
.end method

.method public static b(Ljava/security/Permission;)V
    .locals 2

    invoke-static {}, Ljava/lang/System;->getSecurityManager()Ljava/lang/SecurityManager;

    move-result-object v0

    if-eqz v0, :cond_0

    new-instance v1, LBi/r$c;

    invoke-direct {v1, v0, p0}, LBi/r$c;-><init>(Ljava/lang/SecurityManager;Ljava/security/Permission;)V

    invoke-static {v1}, Ljava/security/AccessController;->doPrivileged(Ljava/security/PrivilegedAction;)Ljava/lang/Object;

    :cond_0
    return-void
.end method

.method public static c(I)I
    .locals 1

    const/16 v0, 0x400

    if-le p0, v0, :cond_3

    const/16 v0, 0x800

    if-gt p0, v0, :cond_0

    const/16 p0, 0xe0

    goto :goto_0

    :cond_0
    const/16 v0, 0xc00

    if-gt p0, v0, :cond_1

    const/16 p0, 0x100

    goto :goto_0

    :cond_1
    const/16 v0, 0x1e00

    if-gt p0, v0, :cond_2

    const/16 p0, 0x180

    goto :goto_0

    :cond_2
    const/16 p0, 0x200

    goto :goto_0

    :cond_3
    const/16 p0, 0xa0

    :goto_0
    return p0
.end method

.method public static d(LBi/r$d;)[Ljava/lang/Object;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "LBi/r$d;",
            ")[TT;"
        }
    .end annotation

    sget-object v0, LBi/r;->b:Ljava/security/Permission;

    invoke-static {v0}, LBi/r;->b(Ljava/security/Permission;)V

    invoke-static {p0}, LBi/r;->m(LBi/r$d;)[Ljava/lang/Object;

    sget-object v0, LBi/r;->g:Ljava/util/Map;

    invoke-static {p0}, LBi/r$d;->a(LBi/r$d;)Ljava/lang/String;

    move-result-object p0

    invoke-interface {v0, p0}, Ljava/util/Map;->remove(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object p0

    check-cast p0, [Ljava/lang/Object;

    return-object p0
.end method

.method public static e(LBi/r$d;)[Ljava/lang/Object;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "LBi/r$d;",
            ")[TT;"
        }
    .end annotation

    sget-object v0, LBi/r;->c:Ljava/security/Permission;

    invoke-static {v0}, LBi/r;->b(Ljava/security/Permission;)V

    invoke-static {p0}, LBi/r;->m(LBi/r$d;)[Ljava/lang/Object;

    move-result-object p0

    return-object p0
.end method

.method public static f()LBi/p;
    .locals 1

    sget-object v0, LBi/r;->i:LBi/p;

    return-object v0
.end method

.method public static g(LBi/r$d;)Ljava/lang/Object;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "LBi/r$d;",
            ")TT;"
        }
    .end annotation

    invoke-static {p0}, LBi/r;->p(LBi/r$d;)[Ljava/lang/Object;

    move-result-object p0

    if-eqz p0, :cond_0

    const/4 v0, 0x0

    aget-object p0, p0, v0

    return-object p0

    :cond_0
    const/4 p0, 0x0

    return-object p0
.end method

.method public static h()Ljava/security/SecureRandom;
    .locals 3

    sget-object v0, LBi/r;->j:Ljava/util/concurrent/atomic/AtomicReference;

    const/4 v1, 0x0

    sget-object v2, LBi/r;->h:LBi/Q;

    invoke-static {v0, v1, v2}, Landroidx/lifecycle/c;->a(Ljava/util/concurrent/atomic/AtomicReference;Ljava/lang/Object;Ljava/lang/Object;)Z

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, LBi/Q;

    invoke-interface {v0}, LBi/Q;->get()Ljava/security/SecureRandom;

    move-result-object v0

    return-object v0
.end method

.method public static i(Ljava/security/SecureRandom;)Ljava/security/SecureRandom;
    .locals 0

    if-nez p0, :cond_0

    invoke-static {}, LBi/r;->h()Ljava/security/SecureRandom;

    move-result-object p0

    :cond_0
    return-object p0
.end method

.method public static j()LBi/p;
    .locals 1

    sget-object v0, LBi/r;->l:Ljava/util/concurrent/atomic/AtomicReference;

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, LBi/p;

    return-object v0
.end method

.method public static k(LBi/r$d;I)Ljava/lang/Object;
    .locals 4
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "LBi/r$d;",
            "I)TT;"
        }
    .end annotation

    invoke-static {p0}, LBi/r;->p(LBi/r$d;)[Ljava/lang/Object;

    move-result-object v0

    const/4 v1, 0x0

    if-nez v0, :cond_0

    return-object v1

    :cond_0
    invoke-static {p0}, LBi/r$d;->b(LBi/r$d;)Ljava/lang/Class;

    move-result-object v2

    const-class v3, LXi/q;

    invoke-virtual {v2, v3}, Ljava/lang/Class;->isAssignableFrom(Ljava/lang/Class;)Z

    move-result v2

    const/4 v3, 0x0

    if-eqz v2, :cond_2

    :goto_0
    array-length p0, v0

    if-eq v3, p0, :cond_4

    aget-object p0, v0, v3

    check-cast p0, LXi/q;

    invoke-virtual {p0}, LXi/q;->f()Ljava/math/BigInteger;

    move-result-object v2

    invoke-virtual {v2}, Ljava/math/BigInteger;->bitLength()I

    move-result v2

    if-ne v2, p1, :cond_1

    return-object p0

    :cond_1
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    :cond_2
    invoke-static {p0}, LBi/r$d;->b(LBi/r$d;)Ljava/lang/Class;

    move-result-object p0

    const-class v2, LXi/z;

    invoke-virtual {p0, v2}, Ljava/lang/Class;->isAssignableFrom(Ljava/lang/Class;)Z

    move-result p0

    if-eqz p0, :cond_4

    :goto_1
    array-length p0, v0

    if-eq v3, p0, :cond_4

    aget-object p0, v0, v3

    check-cast p0, LXi/z;

    invoke-virtual {p0}, LXi/z;->b()Ljava/math/BigInteger;

    move-result-object v2

    invoke-virtual {v2}, Ljava/math/BigInteger;->bitLength()I

    move-result v2

    if-ne v2, p1, :cond_3

    return-object p0

    :cond_3
    add-int/lit8 v3, v3, 0x1

    goto :goto_1

    :cond_4
    return-object v1
.end method

.method public static l(LBi/r$d;)[Ljava/lang/Object;
    .locals 0
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "LBi/r$d;",
            ")[TT;"
        }
    .end annotation

    invoke-static {p0}, LBi/r;->p(LBi/r$d;)[Ljava/lang/Object;

    move-result-object p0

    if-nez p0, :cond_0

    const/4 p0, 0x0

    return-object p0

    :cond_0
    invoke-virtual {p0}, [Ljava/lang/Object;->clone()Ljava/lang/Object;

    move-result-object p0

    check-cast p0, [Ljava/lang/Object;

    return-object p0
.end method

.method public static m(LBi/r$d;)[Ljava/lang/Object;
    .locals 2

    sget-object v0, LBi/r;->f:Ljava/lang/ThreadLocal;

    invoke-virtual {v0}, Ljava/lang/ThreadLocal;->get()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Ljava/util/Map;

    if-nez v1, :cond_0

    new-instance v1, Ljava/util/HashMap;

    invoke-direct {v1}, Ljava/util/HashMap;-><init>()V

    invoke-virtual {v0, v1}, Ljava/lang/ThreadLocal;->set(Ljava/lang/Object;)V

    :cond_0
    invoke-static {p0}, LBi/r$d;->a(LBi/r$d;)Ljava/lang/String;

    move-result-object p0

    invoke-interface {v1, p0}, Ljava/util/Map;->remove(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object p0

    check-cast p0, [Ljava/lang/Object;

    return-object p0
.end method

.method public static varargs n(LBi/r$d;[Ljava/lang/Object;)V
    .locals 2
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "LBi/r$d;",
            "[TT;)V"
        }
    .end annotation

    invoke-static {p0}, LBi/r$d;->b(LBi/r$d;)Ljava/lang/Class;

    move-result-object v0

    const/4 v1, 0x0

    aget-object v1, p1, v1

    invoke-virtual {v1}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/Class;->isAssignableFrom(Ljava/lang/Class;)Z

    move-result v0

    if-eqz v0, :cond_0

    invoke-static {p0, p1}, LBi/r;->o(LBi/r$d;[Ljava/lang/Object;)V

    sget-object v0, LBi/r;->g:Ljava/util/Map;

    invoke-static {p0}, LBi/r$d;->a(LBi/r$d;)Ljava/lang/String;

    move-result-object p0

    invoke-interface {v0, p0, p1}, Ljava/util/Map;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    return-void

    :cond_0
    new-instance p0, Ljava/lang/IllegalArgumentException;

    const-string p1, "Bad property value passed"

    invoke-direct {p0, p1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method public static o(LBi/r$d;[Ljava/lang/Object;)V
    .locals 2
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "LBi/r$d;",
            "[TT;)V"
        }
    .end annotation

    sget-object v0, LBi/r;->f:Ljava/lang/ThreadLocal;

    invoke-virtual {v0}, Ljava/lang/ThreadLocal;->get()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Ljava/util/Map;

    if-nez v1, :cond_0

    new-instance v1, Ljava/util/HashMap;

    invoke-direct {v1}, Ljava/util/HashMap;-><init>()V

    invoke-virtual {v0, v1}, Ljava/lang/ThreadLocal;->set(Ljava/lang/Object;)V

    :cond_0
    invoke-static {p0}, LBi/r$d;->a(LBi/r$d;)Ljava/lang/String;

    move-result-object p0

    invoke-interface {v1, p0, p1}, Ljava/util/Map;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    return-void
.end method

.method public static p(LBi/r$d;)[Ljava/lang/Object;
    .locals 2

    sget-object v0, LBi/r;->f:Ljava/lang/ThreadLocal;

    invoke-virtual {v0}, Ljava/lang/ThreadLocal;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/util/Map;

    if-eqz v0, :cond_1

    invoke-static {p0}, LBi/r$d;->a(LBi/r$d;)Ljava/lang/String;

    move-result-object v1

    invoke-interface {v0, v1}, Ljava/util/Map;->containsKey(Ljava/lang/Object;)Z

    move-result v1

    if-nez v1, :cond_0

    goto :goto_1

    :cond_0
    :goto_0
    invoke-static {p0}, LBi/r$d;->a(LBi/r$d;)Ljava/lang/String;

    move-result-object p0

    invoke-interface {v0, p0}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object p0

    check-cast p0, [Ljava/lang/Object;

    goto :goto_2

    :cond_1
    :goto_1
    sget-object v0, LBi/r;->g:Ljava/util/Map;

    goto :goto_0

    :goto_2
    return-object p0
.end method

.method public static varargs q(LBi/r$d;[Ljava/lang/Object;)V
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "LBi/r$d;",
            "[TT;)V"
        }
    .end annotation

    sget-object v0, LBi/r;->b:Ljava/security/Permission;

    invoke-static {v0}, LBi/r;->b(Ljava/security/Permission;)V

    invoke-virtual {p1}, [Ljava/lang/Object;->clone()Ljava/lang/Object;

    move-result-object p1

    check-cast p1, [Ljava/lang/Object;

    invoke-static {p0, p1}, LBi/r;->n(LBi/r$d;[Ljava/lang/Object;)V

    return-void
.end method

.method public static r(Ljava/security/SecureRandom;)V
    .locals 2

    sget-object v0, LBi/r;->d:Ljava/security/Permission;

    invoke-static {v0}, LBi/r;->b(Ljava/security/Permission;)V

    if-nez p0, :cond_0

    sget-object p0, LBi/r;->j:Ljava/util/concurrent/atomic/AtomicReference;

    sget-object v0, LBi/r;->h:LBi/Q;

    invoke-virtual {p0, v0}, Ljava/util/concurrent/atomic/AtomicReference;->set(Ljava/lang/Object;)V

    goto :goto_0

    :cond_0
    sget-object v0, LBi/r;->j:Ljava/util/concurrent/atomic/AtomicReference;

    new-instance v1, LBi/r$b;

    invoke-direct {v1, p0}, LBi/r$b;-><init>(Ljava/security/SecureRandom;)V

    invoke-virtual {v0, v1}, Ljava/util/concurrent/atomic/AtomicReference;->set(Ljava/lang/Object;)V

    :goto_0
    return-void
.end method

.method public static s(LBi/Q;)V
    .locals 1

    sget-object v0, LBi/r;->d:Ljava/security/Permission;

    invoke-static {v0}, LBi/r;->b(Ljava/security/Permission;)V

    sget-object v0, LBi/r;->j:Ljava/util/concurrent/atomic/AtomicReference;

    invoke-virtual {v0, p0}, Ljava/util/concurrent/atomic/AtomicReference;->set(Ljava/lang/Object;)V

    return-void
.end method

.method public static t(LBi/p;)V
    .locals 1

    sget-object v0, LBi/r;->e:Ljava/security/Permission;

    invoke-static {v0}, LBi/r;->b(Ljava/security/Permission;)V

    if-nez p0, :cond_0

    sget-object p0, LBi/r;->i:LBi/p;

    :cond_0
    sget-boolean v0, LBi/r;->k:Z

    if-eqz v0, :cond_1

    const-string v0, "org.bouncycastle.constraints.allow_override"

    invoke-static {v0}, Lorg/bouncycastle/util/q;->f(Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_2

    :cond_1
    sget-object v0, LBi/r;->l:Ljava/util/concurrent/atomic/AtomicReference;

    invoke-virtual {v0, p0}, Ljava/util/concurrent/atomic/AtomicReference;->set(Ljava/lang/Object;)V

    goto :goto_0

    :cond_2
    sget-object p0, LBi/r;->a:Ljava/util/logging/Logger;

    const-string v0, "attempt to override pre-configured constraints ignored"

    invoke-virtual {p0, v0}, Ljava/util/logging/Logger;->warning(Ljava/lang/String;)V

    :goto_0
    return-void
.end method

.method public static varargs u(LBi/r$d;[Ljava/lang/Object;)V
    .locals 2
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "LBi/r$d;",
            "[TT;)V"
        }
    .end annotation

    sget-object v0, LBi/r;->c:Ljava/security/Permission;

    invoke-static {v0}, LBi/r;->b(Ljava/security/Permission;)V

    invoke-static {p0}, LBi/r$d;->b(LBi/r$d;)Ljava/lang/Class;

    move-result-object v0

    const/4 v1, 0x0

    aget-object v1, p1, v1

    invoke-virtual {v1}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/Class;->isAssignableFrom(Ljava/lang/Class;)Z

    move-result v0

    if-eqz v0, :cond_0

    invoke-virtual {p1}, [Ljava/lang/Object;->clone()Ljava/lang/Object;

    move-result-object p1

    check-cast p1, [Ljava/lang/Object;

    invoke-static {p0, p1}, LBi/r;->o(LBi/r$d;[Ljava/lang/Object;)V

    return-void

    :cond_0
    new-instance p0, Ljava/lang/IllegalArgumentException;

    const-string p1, "Bad property value passed"

    invoke-direct {p0, p1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method public static v(LXi/z;)LXi/q;
    .locals 9

    invoke-virtual {p0}, LXi/z;->b()Ljava/math/BigInteger;

    move-result-object v0

    invoke-virtual {v0}, Ljava/math/BigInteger;->bitLength()I

    move-result v0

    invoke-static {v0}, LBi/r;->c(I)I

    move-result v5

    new-instance v0, LXi/q;

    invoke-virtual {p0}, LXi/z;->b()Ljava/math/BigInteger;

    move-result-object v2

    invoke-virtual {p0}, LXi/z;->a()Ljava/math/BigInteger;

    move-result-object v3

    invoke-virtual {p0}, LXi/z;->c()Ljava/math/BigInteger;

    move-result-object v4

    new-instance v8, LXi/v;

    invoke-virtual {p0}, LXi/z;->d()LXi/C;

    move-result-object v1

    invoke-virtual {v1}, LXi/C;->b()[B

    move-result-object v1

    invoke-virtual {p0}, LXi/z;->d()LXi/C;

    move-result-object p0

    invoke-virtual {p0}, LXi/C;->a()I

    move-result p0

    invoke-direct {v8, v1, p0}, LXi/v;-><init>([BI)V

    const/4 v6, 0x0

    const/4 v7, 0x0

    move-object v1, v0

    invoke-direct/range {v1 .. v8}, LXi/q;-><init>(Ljava/math/BigInteger;Ljava/math/BigInteger;Ljava/math/BigInteger;IILjava/math/BigInteger;LXi/v;)V

    return-object v0
.end method
